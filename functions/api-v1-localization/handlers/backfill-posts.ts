/**
 * Backfill Post Translations Handler
 *
 * Translates posts via batch translation endpoint.
 * Supports two modes:
 * - "full": Offset-based pagination for one-time bulk operations
 * - "incremental": Recent posts only (for cron jobs)
 *
 * Usage:
 ...
 *   "dryRun": false,           // optional, if true just counts posts
 ...
 *   "success": true,
 *   "postsProcessed": 100,
 *   "totalTranslations": 1000,  // 100 posts * 2 fields * 5 locales
...
const JOB_TYPE = "posts";
const STALE_THRESHOLD_MS = 30 * 60 * 1000; // 30 minutes

export default async function backfillPostsHandler(req: Request): Promise<Response> {
  ...
  try {
    ...
    let posts: Array<{ id: number; post_name: string; post_description: string | null }> | null = null;
    ...
    if (onlyUntranslated) {
      ...
      console.log(`Fetching untranslated posts (limit: ${limit}, offset: ${offset})`);
    } else {
      // Build query based on mode
      let query = supabase
        .from("posts")
        .select("id, post_name, post_description", { count: "exact" })
        .eq("is_active", true)
    ...
    if (!posts || posts.length === 0) {
      return new Response(JSON.stringify({
        ...
        message: "No posts found to translate",
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    console.log(`Found ${posts.length} posts to translate (mode: ${effectiveMode}, offset: ${offset}, total: ${count})`);
    ...
    // Trigger batch translation for each post (fire-and-forget)
    const translationPromises = posts.map(async (post) => {
      const fields = [];
      
      if (post.post_name && post.post_name.trim().length > 0) {
        fields.push({ name: "title", text: post.post_name });
      }
      
      if (post.post_description && post.post_description.trim().length > 0) {
        fields.push({ name: "description", text: post.post_description });
      }

      if (fields.length === 0) {
        return; // Skip posts with no content
      }
      ...
      try {
        ...
        if (response.error) {
          console.warn(`Failed to trigger translation for post ${post.id}:`, response.error);
        } else {
          console.log(`Triggered translation for post ${post.id}`);
        }
      } catch (error) {
        console.warn(`Error triggering translation for post ${post.id}:`, (error as Error).message);
      }
    });
    ...
    console.log("Backfill triggered", response);
    ...
    console.error("Backfill error:", (error as Error).message);
